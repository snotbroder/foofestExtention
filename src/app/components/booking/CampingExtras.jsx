...
function CampingExtra() {
  ...
  const twoTentStored = useBasketStore((state) => {
    const twoTentStored = state.campInfo.find(
      (item) => item.itemTitle === "two person tent"
    );
    return twoTentStored.itemMultiply;
  });

  const threeTentStored = useBasketStore((state) => {
    const threeTentStored = state.campInfo.find(
      (item) => item.itemTitle === "three person tent"
    );
  ...
  return (
    <section>
      ...
      <button
        ...
        className={`button mt-6 grid place-self-center lg:place-self-end ${
